<?php
/**
 * Pages des sections — nouveau modèle :
 *  bacentas / centres / cultes / basontas (entités) + nouveaux / generale / bergers (listes).
 */
...
function render_bacentas_grid(): void
{
    ...
    // Un Responsable ne voit que ses propres bacentas.
...
function render_centre_detail(int $centreId): void
{
    ...
    if ($tab === 'suivi') {
        ...
        $content = $tabRow . view('centre_offrandes', [
            'centre'      => $c,
            'monthKey'    => $monthKey,
...
function render_basonta_detail(int $basontaId): void
{
    ...
    // Sélecteur de membre pour ajout (membres non déjà dans le basonta)
    $candidates = qall(
...
/* ================================================================== */
/* LISTES MEMBRES (nouveaux / generale / bergers)                      */
/* ================================================================== */
...
function bacenta_name_map(): array
{
    $map = [];
    foreach (qall('SELECT id, nom FROM bacentas') as $b) {
        $map[(int) $b['id']] = $b['nom'];
    }
    return $map;
}
...
/**
 * Tableau des membres générique.
 * $entityId : id de l'entité (bacenta/centre/culte/basonta) ou null pour une liste.
 */
function members_table(string $section, ?int $entityId, string $label, int $count, ?array $members = null, ?string $filter = null): string
{
    ...
    if ($userNames) {
        foreach (qall('SELECT id, prenom, nom FROM users WHERE id IN (' . implode(',', array_map('intval', array_keys($userNames))) . ')') as $u) {
            $userNames[(int) $u['id']] = full_name($u);
        }
    }
...
/* ================================================================== */
/* SUIVI BACENTA (visites + offrandes)                                 */
/* ================================================================== */
...
function render_member_form(string $section): void
{
    ...
    // Membres (pour invite_par / recu_par)
    $allUsers = qall('SELECT id, prenom, nom FROM users ORDER BY prenom, nom');
    $userOptions = '';
...
function render_bacenta_form(): void
{
    ...
    $centres = get_centres();
    $responsables = qall("SELECT id, prenom, nom FROM users WHERE role = 'responsable' ORDER BY prenom, nom");
...
function render_culte_form(): void
{
    ...
    $responsables = qall("SELECT id, prenom, nom FROM users WHERE role IN ('leader','pasteur','reverant','responsable') ORDER BY prenom, nom");
    $respOpts = '<option value="">— Aucun —</option>';
    foreach ($responsables as $u) {
        $respOpts .= '<option value="' . $u['id'] . '"' . ($c && (int) $c['responsable_id'] === (int) $u['id'] ? ' selected' : '') . '>' . h(full_name($u)) . '</option>';
    }
    $content = view('forms/culte', [
        'culte'     => $c,
        'respOpts'  => $respOpts,
