...
function render_berger_fiche_page(): void
{
    ...
    if ($tab === 'dimes') {
        ...
        $body = view('pages/berger_examens', ['member' => $member, 'examens' => get_examens($memberId)]);
    } elseif ($tab === 'veillees') {
        $veillees = get_veillees($memberId);
        $body = view('pages/berger_veillees', [
            'member'  => $member,
            'veillees'=> $veillees,
            'presentCount' => count(array_filter($veillees, fn($v) => (bool) $v['present'])),
        ]);
    } else {
        $body = view('pages/berger_infos', ['member' => $member]);
    }
...
$charts = null;
    $adminStats = '';
    if ($isAdmin) {
        $series = (new ReportService())->weeklySeries($memberId, current_year());
        $charts = ['suivi' => $series];
        $adminStats = view('pages/suivi_admin_stats', [
            'pctWeek' => $pct,
            'pctYear' => compute_year_completion($memberId, current_year()),
        ]);
    }

    $content = view('pages/suivi_week', [
        'backBtn'    => $backBtn,
        'member'     => $member,
        'weekKey'    => $weekKey,
