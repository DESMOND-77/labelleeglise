...
function render_apropos_page(): void
{
    ...
    if ($form === 'histoire') {
        $p = get_presentation();
        $content = view('pages/forms/histoire', [
            'accroche'  => $p['accroche'] ?? '',
            'histoire'  => $p['histoire'] ?? '',
    ...
    if ($form === 'equipe') {
        ...
        $member = $id ? Query::one('SELECT * FROM equipe WHERE id = ?', [$id]) : null;
        $content = view('pages/forms/equipe', [
            'member'    => $member,
            'isNew'     => !$member,
            'cancelUrl' => url('index.php', ['page' => 'apropos']),
            'csrf'      => csrf_field(),
...
function render_centres_presentation_page(): void
{
    ...
    if (nav('id')) {
        ...
        $content = view('pages/centre_article', [
            'c'       => $c,
            'isAdmin' => $isAdmin,
    ...
    $content = view('pages/centres_presentation', [
        'cards'   => $cards,
        'addCard' => $addCard,
